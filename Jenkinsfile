            ...
            steps {
                script {
                    ...
                    withCredentials([string(credentialsId: 'study-connect-tg-token', variable: 'TOKEN')]) {
                        sh "docker run --restart always --name study-tg-gateway-prod --network master-prod-network -d owa.gigachadus.ru/study-tg-gateway:latest --master.uri=gigachadus.ru --bot.token=$TOKEN"
                    }
                }
            }
        }
